...
5) 更新：
   - 为避免 Trigger 任务长时间阻塞等待（sleep/poke）在容器/调度重启时被 SIGTERM 干掉，
     ...
     - 后续轮询时通过结果表 `qa_daily_vehicle_result` 的完成度（done/expected）判断已全部落盘，
...
# 配置区（尽量少做自由发挥）
# -----------------------------
DAG_ID = "dq_v1_controller"
SCHEDULE = "*/30 * * * *"

# Watermark Variable
VAR_LAST_PROCESSED = "last_processed_shift_date"
# inflight：当前正在处理但尚未确认完成的 shift_date（避免重复触发）
VAR_INFLIGHT = "dq_v1_inflight_shift_date"
...
TBL_RESULT = "dagster_pipelines.qa_daily_vehicle_result"

# Worker DAG
WORKER_DAG_ID = "dq_v1_worker"

# 分片大小：严格要求 5~10
DEFAULT_SHARD_SIZE = int(os.getenv("DQ_VEHICLE_SHARD_SIZE", "8"))
...
with DAG(
    ...
    def compute_plan() -> Dict:
        ...
        if watermark_dt is None:
            # 冷启动：从 ready_max 往前一天作为 watermark，然后本次处理 ready_max（即下一天）
            target = ready_max.to_date_string()
            logger.info(f"[DQ-Controller] Cold start: target_shift_date={target}")
